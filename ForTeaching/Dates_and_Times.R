...
# NOTE: if converting from a number, you will need to specify the origin and possibly apply some scaling. For example, Excel's "Jan 21, 2014" (the default origin in Excel) is represented as 41660 days since Dec 31, 1899, and with some trial-and-error you can get these conversions to work out:
...
# Read in the file “Metro_pH_2008.txt”, which contains pH data for effluent and bypass flows at Syracuse’s Metro Wastewater Treatment Plant:

input1 <- read.delim(file="Metro_pH_2008.txt", header=T, sep="\t")
# \t -- tab separated file
colnames(input1)
  
#  The “read.delim” command reads in a file in a table format (like read.csv), but allows you to identify how the data is formatted (this file is tab delimited, thus the “\t”).
...
# Since numdays1 is a matrix, you can see that the format of the data in the matrix is now characters.  We can also set the column names as:
  
  colnames(numdays1)<-c("Month","Days")
  numdays1
  
# You will now find the average pH over each quarter of the year.  First set up a matrix to contain the output (avg:  first column will be the quarter of the year and the second column with the average pH) and the number of pH measurements in each quarter of the year:
  
  avg <- matrix(0, nrow=4, ncol=2)
  count <- array(0,4)
  
#  Now create a loop that goes through all of the data points, extracts the quarter of the year in which the data was taken, turns this into a numerical value, and sums and counts the pH values in each year:
...
#  Notes that since avg is a matrix, and the quarter is a character, the pH turns into a character.
  #creading data frame instead of matrix 
