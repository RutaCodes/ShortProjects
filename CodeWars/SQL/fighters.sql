...
--However, due to new health and safety regulations, all ki blasts have been outlawed as a potential fire hazard. Any bout that ended with Hadoken, Shouoken or 
--Kikoken should not be counted in the total wins and losses.
...
ON w.id = f.move_id
WHERE NOT w.move = 'Hadoken' AND NOT w.move = 'Shouoken' AND NOT w.move = 'Kikoken'
GROUP BY f.name
ORDER BY won DESC
LIMIT 6;
